...
// ... existing Firebase config setup
...
// 1. AUTH STATE CHANGE: initialize user
onAuthStateChanged(auth, async user => {
  if (user) {
    currentUser = user;
    const idToken = await user.getIdToken();

    // Backend call to init user and fetch sellerID & PIN
    ...
    sellerCard.innerHTML = `
      ...
      <span>Seller ID: <strong>${sellerID}</strong></span>
    `;
...
// 3. PIN SUBMIT: verify PIN
pinSubmit.addEventListener("click", async () => {
  const pin = pinInput.value.trim();
  if (!pin) return alert("Enter your 8‑digit PIN");
...
// 4. KEEP REST OF PRODUCTION-APP.JS HERE (form handling, PayPal, Firestore, etc.)
...
